...
/**
 * This is an implementation of "An O(m) Algorithm for Cores Decomposition of Networks" by Vladimir Batagelj and Matjaz Zaversnik (2003).
 * Given a BinaryMatrix, it will create an ordering of the rows and columns such that the arboricity of the induced graph is minimal. The
 * algorithm runs in linear time and is the basis for improving the performance of various graph related algorithms.
 *
...
public class MatrixDegeneracy {
	...
	/**
	 * Calculates a degeneracy ordering of the rows and columns of matrix and returns it as an integer array.
	 */
	public static int[] computeDegeneracyOrdering(BinaryMatrix matrix) {
	...
	/**
	 * Calculates the number of connections each row/col has in the matrix. Columns are placed in the 2nd half of the array starting at
	 * matrix.getNumRows() offset.
	 */
	public static int[] computeNodeDegrees(BinaryMatrix matrix) {
		int[] connectivityCounts = new int[matrix.getNumRows() + matrix.getNumColumns()];
	...
	private static int[] computeOrdering(int[] connectivityValues, BinaryMatrix matrix) {
		int[] degeneracyOrdering = initializeAscendingArray(matrix.getNumRows() + matrix.getNumColumns());
		...
		List<List<Integer>> degeneracyGroups = computeDegeneracyGroups(connectivity);

		for (int iPosition = 0; iPosition < degeneracyOrdering.length; iPosition++) {
			int firstNonEmptyGroup = computeIndexOfFirstNonEmptyList(degeneracyGroups);
			int chosenPosition = chooseRandomNodePositionInGroup(firstNonEmptyGroup, degeneracyGroups);
			...
			updateDegeneracyOrder(degeneracyOrdering, iPosition, chosenNode);
		}
	...
	/**
	 * Takes a set of degree counts for rows and columns of a matrix and arranges the indices accordingly. For every type of degree count
	 * that exists in the matrix, we have an arraylist that stores the corresponding indices.
	 *
	 */
	public static List<List<Integer>> computeDegeneracyGroups(int[] connectivityValues) {
		List<List<Integer>> degeneracyGroups = new ArrayList<List<Integer>>();
		...
		for (int iNode = 0; iNode < connectivityValues.length; iNode++) {
			...
			degeneracyGroups.get(connectivityValues[iNode]).add(iNode);
		}
	...
	/**
	 * Takes one of the group lists generated by computeDegeneracyGroups and determines the first arraylist in that list of groups that is
	 * non-empty.
	 *
	 */
	public static int computeIndexOfFirstNonEmptyList(List<List<Integer>> lists) {
		int index = 0;
	...
	/**
	 * Chooses one of the indices given in the first non-empty group of the degeneracy group list.
	 */
	public static int chooseRandomNodePositionInGroup(int groupIndex, List<List<Integer>> groups) {
	...
	private static void updateDegeneracyOrder(int[] degeneracyOrdering, int position, int chosenNode) {
		int chosenNodePosition = computeChosenNodePosition(position, degeneracyOrdering, chosenNode);
		swapIndices(degeneracyOrdering, chosenNodePosition, position);
	}

	private static int computeChosenNodePosition(int startPosition, int[] degeneracyOrdering, int chosenNode) {
	...
	private static void swapIndices(int[] list, int index1, int index2) {
